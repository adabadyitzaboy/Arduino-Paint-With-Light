...
SortRoutine::SortRoutine (NS_Rainbow *ns_strip, byte servoPin, byte redLed, byte greenLed, byte blueLed): 
ns_strip(ns_strip), redLed(redLed), greenLed(greenLed), blueLed(blueLed), hand(servoPin){
  swap_delay = 500;
  swap_complete_delay = 1000;
}
void SortRoutine::setup(){
  pinMode(redLed, OUTPUT);
  pinMode(greenLed, OUTPUT);
  pinMode(blueLed, OUTPUT);
  hand.setup();
}
void SortRoutine::sort(byte num_to_sort){
  hand.attach();
  //PrintValues();  
...
boolean SortRoutine::bubble_sort(byte num_to_sort){
...
boolean SortRoutine::swap(uint32_t rgb1, uint32_t rgb2){
  //true when rgb2 > rgb1
...
byte SortRoutine::get_section(byte r, byte g, byte b){
...
void SortRoutine::set_color(byte strip_cell, uint32_t led_rgb, uint32_t strip_rgb){
...
void SortRoutine::swap_colors(byte c1, uint32_t rgb1, byte c2, uint32_t rgb2){
...
void SortRoutine::moveToAndRemove(byte idx, byte &r, byte &g, byte &b){
